...
# - Inhibitory receptors: specific markers expressed
...
cytokine_expression <- cytokine_expression %>%
  dplyr::mutate(
    ...
    # Split marker names into individual labels
...
# Width of each outer marker ring
labelsize <- 0.2
...
plot <- ggplot(cytokine_expression, aes(ymin = min, ymax = max)) +
  ...
  new_scale_fill() +
  # Outer rings: colored by individual markers
  geom_rect(
    aes(
